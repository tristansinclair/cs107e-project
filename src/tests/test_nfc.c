 ...
 * request a firmware response from the pn532 module
 */
...
/* 
 * test_sam_config
 * configure sam mode so the hat is in normal mode
 */
void test_sam_config(void)
{
    ...
    if (pn532_sam_config() == PN532_STATUS_OK)
    {
...
{
    assert(pn532_sam_config() == PN532_STATUS_OK);
...
/* 
 * test_get_block_info
 * prints block data read from card
 */
